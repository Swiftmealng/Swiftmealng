...
export const initializeSocket = (httpServer: HTTPServer): Server => {
  ...
  trackingNamespace.on('connection', (socket: Socket) => {
    Logger.info('Client connected to tracking namespace', { socketId: socket.id });

    // Join a specific order room
    socket.on('join-tracking', (data: { orderNumber: string }) => {
      const { orderNumber } = data;
      socket.join(orderNumber);
      Logger.info('Client joined order tracking', { orderNumber, socketId: socket.id });
    });
    ...
    socket.on('leave-tracking', (data: { orderNumber: string }) => {
      ...
      Logger.info('Client left order tracking', { orderNumber, socketId: socket.id });
    });

    socket.on('disconnect', () => {
      Logger.info('Client disconnected from tracking namespace', { socketId: socket.id });
    });
  });
  ...
  dashboardNamespace.on('connection', (socket: Socket) => {
    Logger.info('Client connected to dashboard namespace', { socketId: socket.id });
    ...
    socket.on('disconnect', () => {
      Logger.info('Client disconnected from dashboard namespace', { socketId: socket.id });
    });
  });
 ...
 */
export const emitLocationUpdate = (orderNumber: string, data: { lat: number; lng: number; timestamp: Date }) => {
  const trackingNamespace = io.of('/tracking');
 ...
 */
export const emitStatusUpdate = (orderNumber: string, data: { status: string; timestamp: Date }) => {
  const trackingNamespace = io.of('/tracking');
 ...
 */
export const emitDelayAlert = (orderNumber: string, data: { delayMinutes: number; reason?: string }) => {
  const trackingNamespace = io.of('/tracking');
 ...
 */
export const emitDashboardDelayAlert = (orderId: string, delayMinutes: number) => {
  const dashboardNamespace = io.of('/dashboard');
  dashboardNamespace.to('dashboard').emit('delay-alert', { orderId, delayMinutes });
