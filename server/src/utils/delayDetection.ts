...
import { emitDelayAlert, emitDashboardDelayAlert } from '../config/socket/socket';
...
export const checkAndHandleDelay = async (orderId: string): Promise<void> => {
  try {
    ...
    const delayMinutes = Math.floor((now.getTime() - estimatedTime.getTime()) / (1000 * 60));
...
export const startDelayMonitoringBackup = (): void => {
  // Run every 5 minutes as backup (main detection is event-based)
  setInterval(async () => {
    try {
      ...
      if (ordersNearDeadline.length > 0) {
        Logger.info('Running backup delay check', { orderCount: ordersNearDeadline.length });
      ...
      Logger.error('Error in backup delay monitoring', { error: error.message });
    }
  }, 5 * 60 * 1000); // Every 5 minutes (backup only)

  Logger.info('Backup delay monitoring started - checking every 5 minutes for orders past deadline');
};
